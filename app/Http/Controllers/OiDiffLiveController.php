...
//        $step = (int) ($tf / 3); // 3->1, 15->5, 30->10
        ...
        $tf = (int) $request->get('tf', 5);
        if ( ! in_array($tf, [5, 10, 15, 30], true)) {
            $tf = 5;
        }
        $step = (int) ($tf / 5); // 5->1, 10->2, 15->3, 30->6
        ...
        if ( ! $quoteDate) {
            ...
            foreach ($days as $day) {
                if ($day->current == 1) {
                    $currentDate = $day->working_date;
                }
                if ($day->previous == 1) {
                    $prevDate = $day->working_date;
                }
            }
...
//        });
        ...
        $snapshots = $allRows->groupBy(function ($row) {
            $ts      = strtotime($row->captured_at);
            $snapped = floor($ts / 300) * 300;

            return date('Y-m-d H:i', $snapped);
        });
