...
class PopulateDailyTrend extends Command {
    ...
    public function handle() {
        ...
        $targetDate  = DB::table( 'nse_working_days' )
                         ->where( 'previous', 1 )->orWhere( 'current', 1 )
                         ->get();
        $currentDay  = '';
        $previousDay = '';

        foreach ( $targetDate as $day ) {
            if ( $day->current === 1 ) {
                $currentDay = $day->working_date;
            }
            if ( $day->previous === 1 ) {
                $previousDay = $day->working_date;
            }
        }

        if ( ! $previousDay ) {
            $this->error( "No working day found for {$previousDay}" );
        ...
        $this->info( "Saved/updated {$saved} records for {$previousDay}" );
